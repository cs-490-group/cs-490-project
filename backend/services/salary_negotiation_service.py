...
def transform_talking_points(points: List[str]) -> List[Dict[str, Any]]:
    """Convert talking point strings into structured objects with metadata.

    Note: Points generated by AI should already have rich detail.
    This just structures them properly for frontend display.
    """
    if not points:
        return []
    ...
    return [
        {
            ...
            "supporting_data": None,  # AI should embed this in the point text itself
            "confidence_level": 8  # Default confidence for AI-generated points backed by research
        }
...
def transform_counter_offers(template_dict: Dict[str, Any]) -> List[Dict[str, Any]]:
    """Convert counter-offer template into structured objects with actionable guidance"""
    if not template_dict:
        template_dict = {}

    fields = template_dict.get("offer_comparison_fields", [])

    # Specific negotiation guidance for each common compensation component
    negotiation_guidance = {
        "Base Salary": {
            "action": "Compare your offer to market data percentiles",
            "tip": "If below market median, request 10-15% increase. If above 75th percentile, hold firm."
        },
        "Signing Bonus": {
            "action": "Ask if salary is fixed; typical range is 10-25% of annual salary",
            "tip": "Use this to offset relocation costs or make up for signing from previous job"
        },
        "Annual Bonus": {
            "action": "Clarify target percentage and what metrics trigger it",
            "tip": "Ensure you understand the metrics and likelihood of achieving them"
        },
        "Equity/Stock Options": {
            "action": "Clarify vesting schedule, cliff period, and total grant value",
            "tip": "Ask about historical stock performance and dilution from future rounds"
        },
        "Bonus/Equity": {
            "action": "Discuss both target percentage and equity grant details",
            "tip": "Understand vesting schedules and refresh grants"
        },
        "Health Insurance": {
            "action": "Review coverage tier, deductibles, and employer contribution",
            "tip": "Calculate total value including vision, dental, mental health benefits"
        },
        "Retirement": {
            "action": "Ask for employer 401k match percentage",
            "tip": "Some companies offer 4-6% match; negotiate if below industry standard"
        },
        "PTO Days": {
            "action": "Negotiate if below 20 days; also ask about remote work flexibility",
            "tip": "Senior roles often have unlimited PTO - ask about this as alternative"
        },
        "Remote Work": {
            "action": "Clarify full-remote, hybrid schedule, or on-site requirements",
            "tip": "Can be worth 10-20% adjustment if you're avoiding commute/relocation"
        },
        "Professional Development": {
            "action": "Request annual budget (typical: $2-5k) and conference attendance",
            "tip": "Valuable for career growth - some companies have conference budgets"
        },
        "Career Growth": {
            "action": "Discuss growth path, promotion timeline, and skill development",
            "tip": "Ask about mentorship, training, and career growth within the company"
        },
        "Job Security": {
            "action": "Understand contract terms, severance, and acceleration clauses",
            "tip": "Ask about equity acceleration if company is acquired"
        }
    }
    ...
    return [
        {
            ...
            "offered_value": "Enter your offer amount",
            "market_value": "Research comparable amounts",
            "suggested_counter": "Use market research to suggest 10-15% above",
            "reasoning": negotiation_guidance.get(field.get("field", ""), {}).get("action", f"Negotiate {field.get('field', 'this component')} strategically"),
...
async def generate_full_negotiation_prep(
    ...
    try:
        ...
        # 4. Generate counter-offer templates (with market context)
