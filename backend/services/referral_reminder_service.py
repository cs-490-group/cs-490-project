...
Handles scheduled reminders to send referral request emails to contacts.
Sends reminders at 1 day before and on the request date with escalating urgency.
"""
...
class ReferralReminderService:
    """Service for generating and sending referral request reminder emails"""
    ...
    def _build_reminder_html(
        ...
        """
        Build HTML email template for referral reminder
        
        Args:
            referral_data: Referral details (company, position, message, etc.)
            contact_info: Contact details (name, email, company, title, etc.)
            hours_until: Hours until request date (24 or 0)
            user_email: Email of the user being reminded
        """
        urgency_class = "urgent" if hours_until == 0 else "upcoming"
        urgency_text = "TODAY - Time to send your referral request!" if hours_until == 0 else "TOMORROW - Prepare to send your referral request"
        urgency_color = "#dc3545" if hours_until == 0 else "#ffc107"
        action_button_text = "Send Referral Request Now" if hours_until == 0 else "View & Prepare Referral Request"
        ...
        company = referral_data.get('company', 'N/A')
        position = referral_data.get('position', 'N/A')
                            ...
                            <div style="display: flex; align-items: center; gap: 15px;">
                                ...
                                <div>
                                    ...
                                    <p style="margin: 5px 0 0 0; color: #666; font-size: 13px;">
                                        {'This is your final reminder before the request date.' if hours_until == 0 else 'You have one day to prepare before sending this referral request.'}
                                    </p>
                                </div>
                            </div>
                        </td>
                        ...
                        <td style="padding: 40px;">
                            ...
                            <div style="background-color: #f9f9f9; border-left: 4px solid #6c757d; padding: 20px; margin-bottom: 25px; border-radius: 4px;">
                                ...
                                <div style="background-color: white; padding: 15px; border-radius: 4px; white-space: pre-wrap; font-size: 14px; line-height: 1.6; color: #333;">
                                    {message if message else '(No personalized message added)'}
                                </div>
                            </div>
                            ...
                            <div style="background-color: #e7f3ff; border: 1px solid #b3d9ff; padding: 20px; margin-bottom: 25px; border-radius: 4px;">
                                ...
                                <ul style="margin: 0; padding-left: 20px; font-size: 14px; line-height: 1.8; color: #333;">
                                    <li>Review the contact information and ensure it's correct</li>
                                    <li>Read through your personalized message</li>
                                    <li>Consider any additional context or notes</li>
                                    <li>When ready, send the referral request email to {contact_name}</li>
                                </ul>
                            </div>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <td style="background-color: #f9f9f9; padding: 20px 40px; text-align: center; border-top: 1px solid #e0e0e0;">
                            <p style="margin: 0; color: #999; font-size: 12px; line-height: 1.6;">
                                🎯 This is a reminder from <strong style="color: #004d7a;">Metamorphosis</strong> - your referral request is scheduled for {('today' if hours_until == 0 else 'tomorrow')}.
                            </p>
...
"""
    ...
    def _build_reminder_text(
        ...
        """Build plain text version of referral reminder"""
        urgency_text = "TODAY - Time to send your referral request!" if hours_until == 0 else "TOMORROW - Prepare to send your referral request"
        ...
        company = referral_data.get('company', 'N/A')
        position = referral_data.get('position', 'N/A')
        message = referral_data.get('message', '(No personalized message added)')
        notes = referral_data.get('notes', '')
...
NEXT STEPS
============================================
1. Review the contact information above
2. Read your personalized message
3. When ready, send an email to {contact_name} at {contact_email}
4. Mark the referral as sent in Metamorphosis
...
"""
    ...
    def send_referral_reminder_email(
        self,
        recipient_email: str,
        referral_data: Dict[str, Any],
        contact_info: Dict[str, Any],
        ...
        """
        Send a referral request reminder email to the user
        
        Args:
            recipient_email: Email address of the user being reminded
            referral_data: Referral details (company, position, message, etc.)
            contact_info: Contact details to send the referral to
            hours_until: Hours until request date (24 or 0)
        ...
        if hours_until == 24:
            subject = f"🎯 Referral Request Reminder: {contact_name} at {company} (Tomorrow)"
        elif hours_until == 0:
            subject = f"⚡ URGENT: Send Your Referral Request Today - {contact_name} at {company}"
        else:
            subject = f"📌 Referral Request Reminder: {contact_name}"
        ...
        text_part = MIMEText(
            self._build_reminder_text(referral_data, contact_info, hours_until),
            "plain"
        )
        html_part = MIMEText(
            self._build_reminder_html(referral_data, contact_info, hours_until, recipient_email),
            "html"
        )
        ...
        try:
            ...
            raise Exception(f"Failed to send referral reminder email: {str(e)}")


# Global instance
referral_reminder_service = ReferralReminderService()
