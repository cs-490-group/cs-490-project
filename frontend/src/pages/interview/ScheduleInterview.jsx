...
const InterviewScheduling = () => {
  ...
  const checkCalendarConflict = (newDateTime, newDuration) => {
    const newStart = new Date(newDateTime);
    const newEnd = new Date(newStart.getTime() + newDuration * 60000);

    for (const interview of interviews) {
      const existingStart = new Date(interview.datetime);
      const existingEnd = new Date(existingStart.getTime() + (interview.duration || 60) * 60000);
  ...
  const handleScheduleInterview = async (formData) => {
    try {
      ...
      const conflict = checkCalendarConflict(formData.datetime, formData.duration);
      
      if (conflict.hasConflict) {
        const conflictDate = new Date(conflict.conflictingInterview.datetime);
        const confirmSchedule = window.confirm(
          `⚠️ Calendar Conflict Detected!\n\n` +
          `This interview overlaps with:\n` +
          `${conflict.conflictingInterview.position} at ${conflict.conflictingInterview.company}\n` +
          `Scheduled for: ${conflictDate.toLocaleString('en-US', { 
            weekday: 'long', 
            month: 'long', 
            day: 'numeric', 
            hour: 'numeric', 
          ...
          `Do you want to schedule this interview anyway?`
        );
